...
EC_FT=EC_FT
EC_PH=EC_PH
Pheno=data.frame(Env=Pheno$env_code,Line=Pheno$line_code,FT=Pheno$FT,PH=Pheno$PH)
...
#EC_FT=read.table("FT_envMeanPara_2_100.txt",header=T,sep="\t");
head(EC_FT)

EC_FT=EC_FT[,-c(1)]
colnames(EC_FT)=c("Env",colnames(EC_FT)[-1])
EC_FT

#EC_PH=read.table("PH_envMeanPara_75_81.txt",header=T,sep="\t");
head(EC_PH)

EC_PH=EC_PH[,-c(1)]
colnames(EC_PH)=c("Env",colnames(EC_PH)[-1])
EC_PH

lettuce0=merge(df_balanced,cbind(Env=EC[,1],scale(EC[,-1])),by="Env") 
lettuce0=lettuce0[order(lettuce0$Env,lettuce0$Genotype),]
head(lettuce0)
lettuceFT=merge(df_balanced,cbind(Env=EC_FT[,1],scale(EC_FT[,-1])),by="Env") 
lettuceFT=lettuceFT[order(lettuceFT$Env,lettuceFT$Genotype),]
head(lettuceFT)

lettucePH=merge(df_balanced,cbind(Env=EC_PH[,1],scale(EC_PH[,-1])),by="Env") 
lettucePH=lettucePH[order(lettucePH$Env,lettucePH$Genotype),]
head(lettucePH)
# Convert relevant columns to numeric
numeric_vars <- c("DL", "GDD", "PTT", "PTR", "PTS")
...
# run the PLS to get z1 and z2
# Convert FV into a matrix for SVD
Y_matrix <- reshape::cast(data = lettuce0[, c("Genotype", "Env", "FT")], 
                          formula = Env ~ Genotype, value = "FT")
Y_data_trn=Y_matrix[,-1]
head(Y_data_trn)
Y_Trn=as.matrix(Y_data_trn)
C_Trn=scale(EC[,-1])
X_Trn=as.matrix(C_Trn)
Y_Mean=apply(Y_data_trn,1,mean)
...
Traits_Names=colnames(lettuce0)[3:4]
Traits_Names
...
#      i=3
 ...
 ##########Models M4 and M5
  ...
  #### Run the BGLR model
    ...
    ###########Model M10
    if(Trait=="FT"){
    lettuce_Trait=lettuceFT
    X2 <- model.matrix(~0 + PTR:X1, data=lettuce_Trait) 
    } else{
      lettuce_Trait=lettucePH
      X2 <- model.matrix(~0 + PTR:X1, data=lettuce_Trait)
    }
...
Summary_All
write.csv(Summary_All,file="All_Methods_Maize0_Final.Final.csv")
